...
#ifndef KAIJU_SUPPORT_CASTING_H
#define KAIJU_SUPPORT_CASTING_H

#include "kaiju/Support/Compiler.h"
#include "kaiju/Support/type_traits.h"
#include <cassert>
#include <memory>
#include <type_traits>
...
//  if (isa<Type>(myVal)) { ... }
//
template <class X, class Y> KAIJU_NODISCARD inline bool isa(const Y &Val) {
...
template <class X, class Y>
KAIJU_NODISCARD inline typename cast_retty<X, Y *>::ret_type
cast_or_null(Y *Val) {
...
template <class X, class Y>
KAIJU_NODISCARD inline typename cast_retty<X, Y>::ret_type dyn_cast(Y &Val) {
  return isa<X>(Val) ? cast<X>(Val) : nullptr;
}
...
template <class X, class Y>
KAIJU_NODISCARD inline auto unique_dyn_cast(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!isa<X>(Val))
    return nullptr;
  return cast<X>(std::move(Val));
...
template <class X, class Y>
KAIJU_NODISCARD inline auto unique_dyn_cast_or_null(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!Val)
    return nullptr;
  return unique_dyn_cast<X, Y>(Val);
